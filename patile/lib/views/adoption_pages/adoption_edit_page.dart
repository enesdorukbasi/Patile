...
class AdoptionEditPage extends StatelessWidget {
  Adoption adoption;

  AdoptionEditPage({Key? key, required this.adoption}) : super(key: key);
...
class AdoptionEditView extends StatelessWidget {
  Adoption adoption;
  Size? size;
  ...
  AdoptionEditView({Key? key, required this.adoption}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    size = MediaQuery.of(context).size;
    context.read<Image1Cubit>().image1Change(adoption.photoOneUrl);
    context.read<Image2Cubit>().image2Change(adoption.photoTwoUrl);
    context.read<Image3Cubit>().image3Change(adoption.photoThreeUrl);
    return Scaffold(
      backgroundColor: Theme.of(context).backgroundColor,
      appBar: AppBars().colorBackgroundAppbar(context, 'İlan Düzenle', null),
      body: Padding(
    ...
    return Form(
        ...
        child: ListView(
          children: [
            ...
            TextFormField(
              initialValue: adoption.title,
              validator: (value) {
                if (value!.length < 3) {
                  return "Başlık alanı minimum 3 karakter içermelidir.";
                }
              },
              ...
              InkWell(
                ...
                child: BlocBuilder<Image1Cubit, String>(
                  builder: (context, state) {
                    return CircleAvatar(
                      radius: 50,
                      backgroundImage: _getImageWidget(state),
                    );
                  },
                ),
              ),
  ...
  Future<void> _shareAdvertMethod(BuildContext context) async {
    try {
      ...
      //Creating Advert
      await AdoptionFirestoreServices().upgradeAdvert(
        title: title,
        content: content,
        il: il,
        ilce: ilce,
        ...
        petBreed: petBreed,
        old: old.toString(),
        photoOneUrl: photo1Url ?? adoption.photoOneUrl,
        photoTwoUrl: photo2Url ?? adoption.photoTwoUrl,
        photoThreeUrl: photo3Url ?? adoption.photoThreeUrl,
      );
  ...
  _getImageWidget(String state) {
    if (state != null || state != "") {
      if (state.contains("http")) {
        return NetworkImage(state);
      } else {
        return FileImage(File(state));
      }
    }
    return null;
  }
